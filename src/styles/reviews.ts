export const reviews = {
  container:
    'relative  bg_img max-w-[1320px] w-full mx-auto px-[25px] max-[768px]:px-[15px]',
  burgerModificator:
    'w-[85px] h-[85px] absolute top-[15px] right-[30px] justify-end',
  logoModificator: 'w-[85px] absolute top-[0] right-[30px] max-[992px]:hidden',
  title:
    'text-white text-h3 font-medium max-[576px]:text-md text-center pt-[150px] mb-[70px] max-[576px]:mb-[50px] max-[576px]:pt-[130px]',
  reviewsSwiperContainer:
    'flex items-center justify-center w-full  py-[20px] max-h-[280px]',
  reviewContainer:
    'reviews__shadow max-w-[400px] w-full mx-auto bg-gold rounded-[30px] p-[30px] max-h-[300px]',
  footerWrapper:
    'max-w-[992px] flex mx-auto justify-between mb-[35px] mt-[50px] items-center max-[768px]:flex-col max-[768px]:gap-[10px] max-[768px]:mt-[50px] max-[768px]:mb-[20px]',
  footerTitle: 'text-white font-medium text-h3 max-[576px]:text-md',
  //   morecommentsPage
  moreWrapper:
    'relative  bg_img max-w-[1320px] w-full mx-auto px-[25px] max-[768px]:px-[15px]',
  morePaginationContainer: 'max-w-[992px] w-full mx-auto pt-[120px]',
  moreReviewContainer:
    'bg-gold rounded-[30px] py-[15px] px-[30px] max-h-[250px] mb-[30px]',
  btnsWrapper: 'flex justify-center gap-[20px] items-center',
  btnMoreAndBack:
    'text-coal text-classic h-[40px]  rounded-[6px] font-semibold block max-w-[200px] w-full mx-auto bg-gold',
};
